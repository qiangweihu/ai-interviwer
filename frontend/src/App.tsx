...
function App() {
  ...
  const preparePlan = async () => {
    const next = await api<Session>("/api/plan", { method: "POST" });
    setSession(next);
  };
  ...
  const nextRound = () => void run(async () => { const next = await api<Session>("/api/session/new", { method: "POST" }); setSession(next); setFeedback(null); setTurns([]); setQuestion(""); await preparePlan(); });
